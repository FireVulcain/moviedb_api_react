...
export default class Reviews extends Component {
    ...
    componentDidMount = () => {
        ...
        fetch(`https://api.themoviedb.org/3/tv/${imdbID}?api_key=${process.env.REACT_APP_MOVIEDB_API_KEY}&append_to_response=reviews`)
            .then((res) => res.json())
            .then((results) => {
                    ...
                    <div className="reviewsContainer">
                        <div className="infoMovies">
                            ...
                            <div>
                                <a href={`/tv/${datas.id}`}>
                                    <h2>
                                        {datas.name} <span>({new Date(datas.first_air_date).getFullYear()})</span>
                                    </h2>
                                </a>

                                <a href={`/tv/${datas.id}`}>← Back to main</a>
                            </div>
                        </div>
